...
from model_util import EarlyStopping, get_loader, generate_mask, onehot, rmsf_get_average_std
import numpy as np
...
def train_and_validate_for_rmsf(model, i, log_dir, data_dir, f, lr=4e-3, batch_size=16, early_stoping=30, sse=False, sse_model=None):  
    ...
    for epoch in range(num_epoch):
        ...
        print(epoch,'**********************')
        model.train(True)
        for i, (train_data, train_sse, train_resid, train_resname, train_rmsf) in enumerate(train_loader):
            ...
            if not sse:   
                ...
                model_rmsf = model(train_data, sse=train_sse, resid=train_resid, resname=train_resname)
            ...
            del train_data, train_sse, train_resid, train_resname, train_rmsf, train_rmsf_mask, model_rmsf, rmsf_pre
        ...
        cor_list = []
        with torch.no_grad():
            for i, (val_data, val_sse, val_resid, val_resname, val_rmsf) in enumerate(val_loader):
                ...
                if not sse:
                    ...
                    model_rmsf = model(val_data, sse=val_sse, resid=val_resid, resname=val_resname)
                ...
                del val_data, val_sse, val_resid, val_resname, val_rmsf, val_rmsf_mask, model_rmsf, rmsf_pre, rmsf_pree, val_rmsff, rmsf_preee, val_rmsfff
        ...
        for i in [1,2,3,4,5]:
            ...
            if sse:
                if sse_model and sse_model_dir:
                    ...
                    cor_list, time_list = test_model_for_rmsf(trained_model, i, log_dir, data_dir, ori_dir, f, sse=True, sse_model=sse_model)
                else:
                    trained_model, sse_model = train_and_validate_for_rmsf(model, i, log_dir, data_dir, f, sse=True,)
                    cor_list, time_list = test_model_for_rmsf(trained_model, i, log_dir, data_dir, ori_dir, f, sse=True)
            else:
                trained_model,sse_model = train_and_validate_for_rmsf(model, i, log_dir, data_dir, f, sse=False)
                cor_list, time_list = test_model_for_rmsf(trained_model, i, log_dir, data_dir, ori_dir, f, sse=False)
...
def main():
    ...
    a = torch.cuda.FloatTensor(350, 1024, 5000)
    b = torch.cuda.FloatTensor(350, 1024, 5000)
    a = a.to(torch.device("cuda:0"))
    b = b.to(torch.device("cuda:1"))
    del a
    del b

    log_dir = "/data/rna_log/gtresall/"
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    record = f"{log_dir}/record.txt"
    data_dir = "/data/input/ad_4040404010/"
