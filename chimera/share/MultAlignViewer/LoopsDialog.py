			...
			for i in range(len(ungappedSeq)):
				if i in matchMap:
					if state == "in missing":
						...
						while end < i+numAdj and end+1 in matchMap:
							end += 1
						if start > 1 or modelingArea != "internal":
							loopData.append((start, end))
						state = "in structure"
				else:
					if state == "in structure":
						start = i+1
						while start > i+1-numAdj and start-2-numAdj in matchMap:
							start -= 1
						state = "in missing"
			if state == "in missing" and modelingArea != "internal":
				loopData.append((start, len(ungappedSeq)))
			if not loopData:
