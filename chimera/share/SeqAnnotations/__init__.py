...
def mapUniprotNameID(ident):
	...
	params = { 'from': 'ACC+ID', 'to': 'ACC', 'format': 'tab', 'query': ident}
	data = urllib.urlencode(params)
	request = urllib2.Request("https://www.uniprot.org/uploadlists/", data)
	...
	# invalid IDs now apparently return the header line with no body lines
	if len(lines) < 2:
		raise InvalidAccessionError("Invalid UniProt entry name / accession"
			" number: %s" % ident)
