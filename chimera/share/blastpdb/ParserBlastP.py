...
class BlastpService:
	...
	ServiceName = "BlastProtein2Service"
	...
	def __init__(self, finishCB, params=None, sessionData=None,
        ...
        def makeArgList(self, db, name, outputFile, evalue, matrix):
		return "-d %s -i %s -o %s -e %s -M %s" % (
				db, name, outputFile, evalue, matrix)
	...
	def _initBlast(self, program, db, queryName, querySeq,
		...
		name = "query.fa"
		inputFile = makeInputFileWithContents(name,
					self.makeFasta(queryName, querySeq))
                argList = self.makeArgList(
				db, name, self.OutputFile, evalue, matrix)
...
include the traceback printed above as part of the problem description."""
	...
	def _statusCB(self):
		...
		if not self.opal.isFinished():
			...
			return
		self.opal.getJobStatistics()
		if self.opal.times[0]:
			self.task.setStartTime(self.opal.times[0])
		if self.opal.times[1]:
			self.task.setEndTime(self.opal.times[1])
		self.task.finished()
		self.task = None
