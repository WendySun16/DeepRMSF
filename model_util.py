    ...
    if not only_rmsf:
        ...
        if not only_sse:
            ...
            if contour:
                ...
                dataset = Data.TensorDataset(intensity, sse,resid, resname, rmsf, con)
            else:
                dataset = Data.TensorDataset(intensity, sse, resid, resname, rmsf)

        else:
            dataset = Data.TensorDataset(intensity, sse, resid, resname)    
    else:
        intensity = data['intensity'].to(device)
        rmsf = data['rmsf'].to(device)
        if contour:
...
def rna_concat_data(file_pre, datalist, dsize=(40,40,40,40,10), train=True, only_rmsf=False, only_sse=False, pre=False):
    ...
    if train:
        if only_rmsf:
            ...
            concat_data = {'intensity': concat_intensity, 'sse': concat_sse, 'resid': concat_res, 'resname': concat_resname, 'rmsf': concat_rmsf}
            return concat_data
