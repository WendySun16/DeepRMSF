...
def rna_pdb2sse(org_pdb, dssr_pairs):
    ...
    with open(org_pdb) as o_pdb:
        ...
        resids = []
    for line in lines:
        if line.split()[0] == 'ATOM' and line.split()[1] == str(serials[num]) and line[21] == chains[num]:
            resid = line[22:27]
            resids.append(resid)
            num += 1
        if num >= len(serials):
            break
    res = zip(list(chains), list(resids))
    r = list(res)
    list_serial = []
    ...
    with open(sselist_file, 'w') as p:
        for serid, resid, SSE, ch, rname in zip(list_serial, list_resid, list_sse, list_c, list_r):
            p.write(f"{serid}            {resid}            {SSE}            {ch}            {rname} \n ")
...
def rna_sse2map3(mrc, pdb, pdbsselist, r=1.5, sim=True):
    ...
    return map2, sse_map, res_map, resname_map, info
...
def rna_select_map_for_split2(box, sse_box, res_box, resname_box, rmsf_box, contour_level, pre=False, th1=0.05, th2=0.005):
    ...
    if max <= threshold:
        ...
        length_c = 20
        length_r = 10
        if sum_s / (length_s * length_s * length_s) < th1 or sum_c / (
                length_c * length_c * length_c) < th1 or sum_res / (length_c * length_c * length_c) < th1 or sum_r / (
                length_r * length_r * length_r) < th2:
            return False
    return True
